...
func (h *WithdrawHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("Withdraw request: transaction_id=%s, amount=%.2f", req.Id, req.Amount)
	...
	withdrawResp, err := h.DBClient.Withdraw(context.Background(), &dbpb.WithdrawRequest{
		Token:         req.Token,
		TransactionId: req.Id,
		Amount:        req.Amount,
		Timestamp:     req.Timestamp,
	})
	if err != nil || withdrawResp.Error != 0 {
		json.NewEncoder(w).Encode(map[string]interface{}{"error": 1, "balance": 0})
		return
	}
	...
	_, _ = h.IdempotencyClient.SaveTransaction(context.Background(), &idpb.SaveTransactionRequest{
		UserId:        userID,
		TransactionId: req.Id,
		Balance:       withdrawResp.Balance,
	})

	json.NewEncoder(w).Encode(map[string]interface{}{"error": 0, "balance": withdrawResp.Balance})
}
